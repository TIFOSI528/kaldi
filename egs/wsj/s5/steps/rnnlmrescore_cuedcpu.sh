...
for f in $oldlang/G.fst $rnndir/rnnlm.txt $data/feats.scp $indir/lat.1.gz; do
...
if [ $stage -le 6 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/rnnlm_compute_scores.JOB.log \
    utils/rnnlm_compute_scores_cued.sh $rnndir $adir.JOB/temp $adir.JOB/words_text $adir.JOB/lmwt.rnn \
    || exit 1;
fi
if [ $stage -le 7 ]; then
...
/qwork3/kqy90/kaldi/egs/decode/score_check.sh $scoring_opts --cmd "run.pl -l hostname=Banana" $data $oldlang $dir || exit 1;
